...
from punishment_sim.persistent import PersistentSimulation as TreeEngine, CounterForkPolicy, UnsupportedStateError
from punishment_sim.ostracism import OstracismPolicy
from punishment_sim.selfish_counter import SelfishState, SelfishStateError
...
MODELS = {"counter_fork": "persistent-counter-fork-v2", "ignore": "persistent-ignore-v2",
          "selfish": "persistent-selfish-counter-v2"}
...
def eligible_tips(sim, actor, visible):
    ...
    if isinstance(p, CounterForkPolicy):
        ...
        choices = [None, *(b for b in visible if b not in p.rejected_by)]
    else:
        choices = None
    if choices is None:
        choices = [None, *visible]
    if not choices:
