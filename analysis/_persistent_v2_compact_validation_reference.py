...
from punishment_sim.persistent_v2_policies import CounterForkPolicy, PettyPolicy
from punishment_sim.persistent_v2_index import initialize_indexes, advance_frontier, index_publication
...
from punishment_sim.persistent_v2 import (PersistentSimulation, CommonRandom, SelfishActors, Publications,
...
def _validate_run(result, population, rule, repetition, strategy, flagged, coalition):
    ...
    for event in range(1, events+1):
        ...
        if not production:
            same(result["public_events"][event-1], expected, "public event/propagation mismatch")
        if "trace" in result:
