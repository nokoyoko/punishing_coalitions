# Frozen pre-optimization v2 oracle; only for local tests/benchmarks.
...
from punishment_sim.persistent import CounterForkPolicy
from punishment_sim.ostracism import OstracismPolicy
from punishment_sim.selfish_validation import replay_reactions
from punishment_sim.selfish_strategy import release_plan
from punishment_sim.persistent_checkpoint import canonical_json, digest, atomic_json
from analysis._persistent_v2_reference import (PersistentSimulation, CommonRandom, SelfishActors, Publications,
...
class PublicReplay(PersistentSimulation):
    ...
    def __init__(self, population, rule, identity, records):
    ...
    def apply_batch(self, batch):
        ids = batch["blocks"]
        self.public.update(ids)
        self.public_tips = (self.public_tips | set(ids)) - {self.blocks[b].parent_id for b in ids}
        self.public_height = max(self.height(b) for b in self.public_tips)
        self.longest_tips = {b for b in self.public_tips if self.height(b) == self.public_height}
        self._check_target_tips(self.longest_tips)
...
def _validate_run(result, population, rule, repetition, strategy, flagged, coalition):
    ...
    same(result["identity"], identity, "identity mismatch")
    ...
    same(result["population"], asdict(replace(population, seed=identity["actual_seed"])), "population/seed mismatch")
    ...
    view = PublicReplay(population, rule, identity, blocks)
    ...
    traces = []
    for event, log in enumerate(result["public_events"], 1):
        ...
        same(log, expected, "public event/propagation mismatch")
        if "trace" in result:
    ...
    from punishment_sim.persistent import Block
    view.blocks = {bid: Block(**vars(b)) for bid, b in view.blocks.items()}
    same(terminal, view.terminal_state(), "terminal/frontier/boundary mismatch")
    same(result["rng"], view.rng_snapshot(), "RNG state/consumption mismatch")
    same(result["episodes"], view.punishment.history if view.punishment else [], "policy history mismatch")
    same(result["selfish_reactions"], view.selfish.history, "missing or incorrect selfish reaction history")
    same(result["reorganizations"], view.reorganizations, "reorganization mismatch")
    same(result["member_activations"], dict(view.activations), "activation accounting mismatch")
    same(result["member_opportunities"], dict(view.opportunities), "opportunity accounting mismatch")
    same(result["natural_pairs"], dict(view.natural_pairs), "natural pair mismatch")
    require(set(result["actors"]) == set(miners), "actor coverage")
    for a, miner in miners.items():
        accepted = view.rewards[a]
        orphaned = sum(blocks[b]["owner_id"] == a and b not in view.canonical_chain for b in public)
        ...
        same(result["actors"][a], expected, "actor/reward ledger mismatch")
    if "trace" in result:
        same(result["trace"], traces, "trace mismatch")
...
class ConditionStore:
    ...
    def path(self, identity):
        return self.directory / ("baselines" if not identity["flagged"] else "conditions") / (digest(identity)+".json")
    ...
    def load(self, population, rule, repetition, strategy, flagged, coalition):
        ...
        schema = CONDITIONAL_SCHEMA if identity["flagged"] else BASELINE_SCHEMA
    ...
    def save(self, result, population, rule, repetition, strategy, flagged, coalition):
        ...
        identity = condition_identity(population, repetition, strategy, flagged, coalition, rule)
        payload = {"schema": CONDITIONAL_SCHEMA if identity["flagged"] else BASELINE_SCHEMA,
